...
export default Service.extend({
  ...
  checkRole(role, userRole) {
    let md5Role = this.getMD5(role + this.hash);
    if (Ember.isEmpty(userRole)) return false;
    return md5Role.toLowerCase() == userRole.toLowerCase();
  },
  ...
  checkRoles(roles, userRole) {
    ...
    roles.forEach((role) => {
      let md5Role = this.getMD5(role + this.hash);
      if (md5Role.toLowerCase() == userRole.toLowerCase()) isRole = true;
    });
    return isRole;
  },
});
